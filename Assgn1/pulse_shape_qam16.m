...
function [] = pulse_shape_qam16(symbols, rc_filter)

    % upsampling the data
    qam16_up = upfirdn(symbols, 1, 12);
    % convolution with the desired pulse
    qam16_shaped = conv(qam16_up, rc_filter, 'same');

    in_phase = real(qam16_shaped);
    quadrature = imag(qam16_shaped);
    ...
    subplot(3, 1, 2);
    plot(quadrature, 'r', 'LineWidth', 1.5);
    ...
    sgtitle('16QAM with RC Pulse Shaping');
end
