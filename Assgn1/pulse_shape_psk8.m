...
function [] = pulse_shape_psk8(symbols, rc_filter)
    
    % upsampling the data
    psk8_up = upfirdn(symbols, 1, 12);
    % convolution with the desired pulse
    psk8_shaped = conv(psk8_up, rc_filter, 'same');

    in_phase = real(psk8_shaped);
    quadrature = imag(psk8_shaped);
    ...
    sgtitle('8PSK with RC Pulse Shaping');
end
